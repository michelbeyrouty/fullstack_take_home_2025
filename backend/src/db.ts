...
let db = sqliteOpen({ filename: ":memory:", driver: Database });

export const sql = <T = any>(...args: Parameters<Database["all"]>) =>
  db
    .then((dbo) => dbo.all<T[]>(...args))
    .catch((e) => {
...
export async function safeFreshTestDb() {
  await (await db).close();
  db = sqliteOpen({ filename: ":memory:", driver: Database });
  await initDb();
}
